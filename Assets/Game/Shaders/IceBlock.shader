...
Shader "Custom/IceBlock" 
{
	...
	SubShader 
	{
		...
		void surf (Input IN, inout Ill_SurfaceOutput o) 
		{
			...
			// DX9's Y is opposite of everything else, so if we're in DX9, then flip the Y
			...
			float alpha = 0.2f;
			float behindTexture = tex2D (_GrabTexture, refractionUV).rgb * (1-alpha);
			o.Emission = behindTexture;
