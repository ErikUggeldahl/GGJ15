...
public abstract class CustomMaterialEditor : MaterialEditor
{
	public class FeatureToggle
	{
		...
		public bool Enabled;
		// Secondary toggles hide things that they own if disabled, but do not list them under their checkbox.
		...
		public FeatureToggle(string InspectorName, string InspectorPropertyHideTag, string ShaderKeywordEnabled, string ShaderKeywordDisabled, bool SecondaryToggle)
		{
			this.InspectorName = InspectorName;
			this.InspectorPropertyHideTag = InspectorPropertyHideTag;
			this.ShaderKeywordEnabled = ShaderKeywordEnabled;
	...
	public override void OnInspectorGUI ()
	{
		...
		if (isVisible && !theShader.hasMultipleDifferentValues && theShader.objectReferenceValue != null)
		{
			...
			EditorGUIUtility.labelWidth = Screen.width - controlSize * 2;
			...
			// Draw Secondary toggles
			for (int s = 0; s < Toggles.Count; s++)
			{
				if (Toggles[s].SecondaryToggle)
				{			
					Toggles[s].Enabled = EditorGUILayout.Toggle(Toggles[s].InspectorName, Toggles[s].Enabled);
